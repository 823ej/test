...
function generateUniqueNumbers() {
    ...
    while (uniqueNumbers.length < 3) {
        let n = Math.floor(Math.random() * 22); // 0부터 21까지의 랜덤 숫자 생성
...
window.onload = async function() {
    ...
    console.log(tarotData); // 데이터를 콘솔에 출력하여 확인
    console.log(num); // num 배열을 콘솔에 출력하여 확인
};
...
function cardClick(cardIndex) {
    ...
    console.log(`Card Index: ${cardIndex}, Num: ${num[cardIndex]}`); // 클릭한 카드의 인덱스와 숫자를 콘솔에 출력하여 확인

    const cardData = tarotData[num[cardIndex]];

    if (!cardData) {
        console.error(`No data found for card index ${num[cardIndex]}`);
        return;
    }

    const direction = isReversed ? "reversed" : "upright"; // 방향 설정
    textElement.innerText = cardData[direction][timePeriods[cardIndex]]; // 타로 데이터에 맞는 텍스트 설정
